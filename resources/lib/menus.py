...
class Donate(xbmcgui.WindowDialog):
    def __init__(self):
        ...
        self.text = xbmcgui.ControlLabel(455, 570, 1280, 25, AutoTranslate.language('If you like this add-on, support via PIX above'), textColor='white')
        self.text2 = xbmcgui.ControlLabel(535, 600, 1280, 25, AutoTranslate.language('Press BACK to exit'), textColor='white')
...
class thunder(myAddon):
    ...
    def is_inputstream_available(self):
        try:
            available = xbmc.getCondVisibility('System.HasAddon(inputstream.adaptive)')
            return available
        except Exception:
            return False
    ...
    def is_ffmpegdirect_available(self):
        try:
            available = xbmc.getCondVisibility('System.HasAddon(inputstream.ffmpegdirect)')
            return available
        except Exception:
            return False
            ...
            if mediatype == 'movie':
                if imdb:
                    ...
                    for name2, page_href in menus_links:
                        self.addMenuItem({'name': name2, 'action': 'play_resolve', 'video_title': title, 'url': page_href, 'iconimage': iconimage, 'fanart': fanart, 'playable': 'false', 'description': description, 'imdbnumber': imdb, 'year': year, 'mediatype': 'video'}, folder=False)
                    self.end()
                else:
                ...
                for season in seasons:
                    ...
                    season_name = "Especiais" if season_number == 0 else f"{season_number}ª temporada"
                    icon = f"https://image.tmdb.org/t/p/w500{season.get('poster_path')}" if season.get('poster_path') else iconimage
                    self.addMenuItem({'name': season_name, 'action': 'season_tvshow', 'video_id': video_id, 'year': year, 'iconimage': icon, 'fanart': fanart, 'description': season.get('overview', ''), 'season': str(season['season_number']), 'mediatype': 'season'}, folder=True)
                self.end()
        except Exception:
    ...
    def season_tvshow(self, video_id, year, season):
        ...
        for episode in src.get('episodes', []) or []:
            ...
            self.addMenuItem({'name': f"{int(season)}x{int(epnum):02d} - {ep_name}", 'action': 'provider', 'video_id': video_id, 'year': year, 'season': str(season), 'episode': str(epnum), 'iconimage': icon, 'fanart': fanart, 'description': description, 'imdbnumber': imdb, 'title': title_show, 'video_title': f"{title_show} - {ep_name}", 'mediatype': 'episode'}, folder=True)
        self.end()
    ...
    def list_server_links(self, imdb, year, season, episode, name, video_title, genre, iconimage, fanart, description):
        ...
        if menus_links:
            self.setcontent('videos')
            for name2, page_href in menus_links:
                self.addMenuItem({'name': name2, 'action': 'play_resolve', 'video_title': video_title, 'url': page_href, 'iconimage': iconimage, 'fanart': fanart, 'playable': 'false', 'description': description, 'imdbnumber': imdb, 'season': str(season) if season is not None else '', 'episode': str(episode) if episode is not None else '', 'genre': genre, 'year': str(year) if year is not None else ''}, folder=False)
            self.end()
        else:
            self.notify_no_sources()
    ...
    def resolve_links(self, url, video_title, imdb, year, season, episode, genre, iconimage, fanart, description, playable):
        try:
            # Buscar descrição direto da API (garante que OSD terá sinopse)
            if imdb:
                try:
                    if season and episode:
